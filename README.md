...
在该层中 `adapter` 包需要关注，这个包中存放的是与领域层无关的外部接口调用，也正是架构图中体现的 `NonDomainAdapter`，用于应用层对外部接口的防腐；除此之外，我们还需要关注下 `ability` 包，在一些其他文章中会提到 "能力点" 这个概念，即把一个个能力（如订单创建、订单取消等逻辑）封装起来作为能力点，以实现复用。在最理想的情况下，应用层定义的 `Service` 做的是对能力点的编排。这听起来非常好，但是有一个问题比较难解决：当对这些能力点进行编排时，各个能力点之间若存在相同的查询，可能造成数据的重复查询，导致性能的降低和前后查询数据可能不一致的问题。如果要解决这个问题，便需要通过上下文在各个能力点之间进行传参，线程间的通讯是解决这个问题的难点，为了不增加理解和开发成本，便省略了这一点。

#### 查询应用层（application-query）

这一层在大多数的领域驱动设计的文章中并没有提及，单独提出来这一层是为了完成 CQRS 的模型分离，也就是说查询相关的逻辑与写相关的业务逻辑不共用一套领域模型，而是直接使用数据库中的 PO 模型，这样的作用是能够让开发者更加方便的封装查询结果，降低开发量。
...
在该层中，`assembler` 包需要理解，它与 `convert` 包不同，`convert` 做的仅仅是类型转换和字段映射，而 `assembler` 则是将领域模型转换为应用层需要的 DTO（数据传输对象），它可以做一些业务逻辑处理，比如将领域模型中的金额转换为可读的格式等，用于查询接口结果对象的"组装"。

注意这一层为了直接使用数据模型便依赖了 `flash-sale-infrastructure` 层，同时为了不和领域层耦合，将 `domain` 层排除了，如下：
...
领域层不依赖其它层，是独立于技术和基础设施的"业务"实现。领域层中会依赖抽象，包括仓库、外部接口调用或技术组件等，但并不对此做实现，具体实现由基础设施层去做，体现了 **依赖倒置** 的思想。其中 `adapter` 包下主要为与领域服务相关的外部接口调用适配器，同样它起到的作用也是防腐；`event` 包下定义了事件模型和事件发送器，事件发送器的具体实现在基础设施中实现，可以是消息队列或事件总线等等。
...
落到编码层面，在写数据侧可能需要通过 ORM 等工具完成对聚合根的持久化，但是在读数据侧则不见得，我们甚至可以通过直接的 SQL 语句从数据库中加载所需查询的数据，当然不推荐这么做，只要我们能够像在 MVC 架构下 “一气呵成” 的开发也满足了我们的诉求，所以才有了上述 `flash-sale-application-query` 层的存在。
